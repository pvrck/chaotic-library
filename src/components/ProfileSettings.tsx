...
import { User, Key, Sparkles, AlertCircle } from 'lucide-react';
...
// Notre catalogue d'avatars gratuits et thématiques
const AVAILABLE_AVATARS = [
  { emoji: '📖', label: 'Grimoire' },
  { emoji: '🧙‍♀️', label: 'Sorcière' },
  { emoji: '🐉', label: 'Dragon' },
  { emoji: '🦉', label: 'Chouette sage' },
  { emoji: '🧪', label: 'Potion' },
  { emoji: '👑', label: 'Couronne' },
  { emoji: '🐈‍⬛', label: 'Chat Noir' },
  { emoji: '🦊', label: 'Renard Errant' },
  { geometry: '✨', emoji: '⚔️', label: 'Guerrière' },
];
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700/50 shadow-xs flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className="text-center sm:text-left space-y-1">
          ...
          <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 flex items-center gap-1 justify-center sm:justify-start">
            <Sparkles className="h-3 w-3" /> Niveau {Math.floor((profile?.xp || 0) / 1000) + 1} (
            {profile?.xp || 0} XP)
          </p>
        </div>
      </div>
